...
const Contact = () => {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      try {
        const response = await fetch("https://rogue0707.com/api/get");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.post("https://rogue0707.com/api/remove", { id });
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-4 px-2">
      <h1 className="text-2xl font-semibold text-center mb-10 text-gray-800 shadow-md bg-white py-5 rounded-lg">
